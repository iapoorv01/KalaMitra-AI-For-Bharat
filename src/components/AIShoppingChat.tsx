  ...
  return (
    <>
      ...
      <motion.button
        ...
        className={`fixed bottom-4 right-4 z-50 w-11 h-11 md:w-12 md:h-12 bg-gradient-to-r from-orange-500 to-red-500 text-white rounded-full shadow-2xl hover:shadow-orange-500/50 transition-all duration-300 flex items-center justify-center group hover:scale-110 ${highlight ? 'ring-4 ring-yellow-400 animate-pulse' : ''}`}
        whileHover={{ rotate: 15 }}
        whileTap={{ scale: 0.95 }}
        onMouseEnter={() => highlight && setShowTooltip(true)}
        onMouseLeave={() => highlight && setShowTooltip(false)}
      >
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="fixed bottom-16 right-4 z-40 w-[92vw] max-w-sm h-[60vh] max-h-[480px] bg-white dark:bg-gray-900 rounded-2xl shadow-2xl flex flex-col overflow-hidden border border-gray-200 dark:border-gray-700"
          >
            {/* Header */}
